...
export const authenticateToken = (
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    res
      ...
      .json({
        message:
          "Neautorizované: Nebol poskytnutý token alebo je neplatná schéma",
      });
    return;
  }
  ...
  if (!token) {
    res
      .status(401)
      .json({ message: "Neautorizované: Nebol poskytnutný token" });
    return;
  }
  ...
  try {
    ...
    if (error instanceof TokenExpiredError) {
      ...
      res
        .status(403)
        .json({ message: "Forbidden: Invalid or malformed token" });
      return;
    }
    ...
    res
      .status(403)
      .json({ message: "Zakázané: Neplatný alebo expirovaný token" });
  }
};
