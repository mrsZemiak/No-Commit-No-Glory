        ...
        <tr v-for="(work, index) in works" :key="index">
          ...
          <td>
              <span :class="work.reviewed ? 'badge success' : 'badge warning'">
                {{ work.reviewed ? "Reviewed" : "Not Reviewed" }}
              </span>
          </td>
          <td>
            <button @click="viewReview(work)" class="btn info">See Review</button>
            <button @click="editWork(work)" class="btn warning">Edit</button>
          </td>
        </tr>
        </tbody>
      </table>
